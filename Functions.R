...
### qxt UP TO EXTREME AGE VIA EXTRAPOLATION
...
#### SIMULATED qxt UP TO EXTREME AGE VIA EXTRAPOLATION
...
### Present value of annuities from a vector of death probabilities q(x) for a specific cohort
annuity.coh <- function(q,v_vect){
#q <- q_LC_BELf.1955
p <- 1-q
p0n <- cumprod(p)
E0n <-  p0n*v_vect[1:length(q)]
ann0 <-  cumsum(E0n) # matrix of present values of annuity starting from age 0
annx <- as.vector(c(rep(0,length(q))))
names(annx)=names(q)
annx[1] <- ann0[length(q)]
for (j in 2:length(q)){
      annx[j] <- (ann0[length(q)]-ann0[j-1])/E0n[j-1] # matrix of present values of annuity starting from age x
}
return(annx)
}

### Present value of life insurances from a vector of death probabilities q(x) for a specific cohort
...
LI0 <-  cumsum(n1A0) # matrix of present values of annuity starting from age 0
LIx <- as.vector(c(rep(0,length(q))))
names(LIx)=names(q)
LIx[1] <- LI0[length(q)]
for (j in 2:length(q)){
      LIx[j] <- (LI0[length(q)]-LI0[j-1])/E0n[j-1] # matrix of present values of annuity starting from age x
}
return(LIx)
}


### Present value of annuities from N.sim vectors of death probabilities q(x) for a specific cohort
...
##### Present value of life insurances from N.sim vectors of death probabilities q(x) for a specific cohort
